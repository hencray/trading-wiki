...
*Status: **Phase 1 complete, Phase 2A v0.1 + v0.2 + v0.2.1 + v0.2.2 SHIPPED. v0.3 backlog item 1 of 6 (Pass 1 ↔ Pass 2 contamination check) SHIPPED 2026-05-10 via a new `trading_wiki/extractors/pass2/ablation.py` harness. Both Pass 2 extractors now accept optional `prompt_path` / `prompt_version` / `persist` kwargs (defaults preserve locked v1 behavior).** Verdict at n=10/10/10: **keep the priming sentence in both v1 prompts** (Concept extractor changed on 10/10 chunks under blind prompt — 5 different concept counts, 5 phrasing/related_terms drifts; TE was more robust at 5/10 identical but had field-level drift on a few chunks including one price-scale anomaly worth a follow-up spot-check). **Pass 1 routing labels are sound** at n=10 — the single routing-arm "miss" was blind-prompt over-extraction on a strategy-talk chunk, not a Pass 1 mislabel. Review-UI dev tool shipped 2026-04-26 as a separate slice. Tier 1 corpus = 12 videos / 95 chunks / 12 trade_examples / 112 concepts at locked `pass1-v1`, `pass2-trade-example-v1`, `pass2-concept-v1`. Pass 2 idempotency fix shipped 2026-04-29 via `pass2_runs` table. Review UI: local Streamlit page at `trading_wiki/review/app.py`. 237 unit tests + 3 integration tests.*
...
**Status:** 🟠 In progress (Phase 2A v0.1 + v0.2 + v0.2.1 + v0.2.2 SHIPPED 2026-04-25 / 2026-04-26 / 2026-04-28 / 2026-04-29; v0.3 backlog items 1 and 2 of 6 — Pass 1 ↔ Pass 2 contamination check + TE silent-price-rescaling audit & prompt v2 — both SHIPPED 2026-05-10; corpus = 12 videos / 95 chunks / 12 trade_examples / 112 concepts; v0.3 remainder = Strategy/Setup/Rule/MarketCondition + Concept dedup + 4 remaining prompt-iteration backlog items, plus a corrective follow-up slice for the 5 baseline TE rows the audit flagged)
...
### Phase 2
- **Phase 2A v0.3 backlog item 2 of 6 = TE silent-price-rescaling audit + prompt v2 — SHIPPED 2026-05-10.** New `trading_wiki/extractors/pass2/price_audit.py` is a read-only audit that walks each locked-v1 TradeExample row's price fields and scans the source chunk text for literal + ×10/÷10/×100/÷100 variants of each value (digit-boundary regex that rejects `295` inside `2950`, `2.95` inside `2.957`, and integer matches followed by `.<digit>`; `$`-stripping + thousands-comma normalization). Severity: literal-present → info; literal-absent + any rescaled-present → high; nothing-present → medium. Findings to gitignored `data/audits/<run_id>/{findings.json,findings.md}`. Audit run on the 12-row v1 baseline: **3 high-severity findings** (te_id=6 chunk_id=12 entry_price `12.5` and target_price `14.45` are ×100 silent rescalings of `1250`/`1445` the speaker said; te_id=2 chunk_id=26 entry_price `295.0` is a ÷100 silent rescaling of `2.95`) and **2 medium-severity findings** (te_id=1 chunk_id=24 target_price `41.11` and te_id=12 chunk_id=74 entry_price `346.6` are values not in their chunk text at all — paraphrase or fabrication). Prompt revision (`prompts/pass2_trade_example_v2.md`) adds one explicit "Preserve numeric scale" bullet under Edge cases (priming sentence — proven load-bearing by v0.3 #1 — untouched). Ablation harness (`ablation.py`) generalized with `--arms te,concept,routing` filter and per-arm `--{te,concept}-test-prompt-{path,version}` overrides defaulting to the existing blind paths so the contamination use case is unchanged. v2 validated by re-running the harness with `--arms te --te-test-prompt-path prompts/pass2_trade_example_v2.md --te-test-prompt-version pass2-trade-example-v2 --n-priming-te 10 --seed 42` (same stratified sample / seed as v0.3 #1; cost $0.11). **Verdict at n=10: pass — v2 locked at `pass2-trade-example-v2`.** Material results in the sample: chunk 26 corrected from anomalous `295.0` to literal `2.95`; chunk 74 declined to extract the fabricated price (count_changed = 1, the correct refusal per acceptance criterion #2); chunk 9 captured a `target_price=333.0` the speaker explicitly stated that v1 missed; chunk 6 dropped a misattributed options-strike-as-target (`180.0`); chunk 8 dropped an invented year (`'2024-06-16'` → NULL for "June 16th" alone). No new fabrications introduced by v2. Chunk 12 (the second high-severity anomaly) was outside the n=10 sample, so its baseline rows remain wrong — those 3 wrong fields plus the 2 medium-severity rows are **carried to a follow-up corrective slice** (re-extract or hand-fix the 5 affected rows; design and scope TBD in that slice). Tests: 279 unit + 3 integration.
