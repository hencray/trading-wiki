...
*Status: **Phase 1 complete, Phase 2A v0.1 + v0.2 + v0.2.1 + v0.2.2 SHIPPED. v0.3 backlog items 1 and 2 of 6 BOTH SHIPPED 2026-05-10.** Item 1 (Pass 1 ↔ Pass 2 contamination check) delivered a new `trading_wiki/extractors/pass2/ablation.py` harness; verdict: keep the priming sentence in both v1 prompts, Pass 1 routing labels are sound. Item 2 (TE silent-price-rescaling audit + prompt v2) delivered `trading_wiki/extractors/pass2/price_audit.py` and `prompts/pass2_trade_example_v2.md`; audit surfaced silent-rescaling anomalies across a subset of TE rows; prompt v2 locked at `pass2-trade-example-v2` after ablation validation. Both Pass 2 extractors now accept optional `prompt_path` / `prompt_version` / `persist` kwargs (defaults preserve locked v1 behavior). Review-UI dev tool shipped 2026-04-26 as a separate slice. Tier 1 corpus = 12 videos / 95 chunks / 12 trade_examples / 112 concepts at locked `pass1-v1`, `pass2-trade-example-v1`, `pass2-concept-v1`. Pass 2 idempotency fix shipped 2026-04-29 via `pass2_runs` table. Review UI: local Streamlit page at `trading_wiki/review/app.py`. 279 unit tests + 3 integration tests.*
