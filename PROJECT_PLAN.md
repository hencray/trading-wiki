...
*Status: **Phase 1 complete. Phase 2 MVP COMPLETE end-to-end: Phase 2A (Pass 1 + Pass 2 all 6 entity types + Pass 3 Concept resolution MVP + Pass 4 relationships) + Phase 2B (review queue + hard-gate checks) + Phase 2C v2 (chart extraction from video scene-change frames) — all SHIPPED 2026-05-11. Polish carried forward: generalize Pass 3 for non-Concept entities, Phase 2C scene-threshold tuning, Phase 2C v1 (PDF/EPUB/Discord — pending image-source content ingest).** Items 1 + 2 + corrective slice details below. Item 3 (trade-attribution semantics) shipped 2026-05-11 as an **investigation** — surveyed the v1 corpus for multi-trade-mention chunks (only chunk 13 found; correctly attributed under v2 — 2 distinct CRWV trades captured with no price field mixups). No prompt change; v2 rules adequate for the current corpus. **LLM-assisted attribution audit infrastructure deferred to v0.4** — building it on a corpus with one multi-trade case is bad ROI; revisit when more multi-trade content lands. Per memory's `small_n_prompt_iteration` rule: lock prompts and carry findings as backlog, don't rewrite on a tiny n. Item 1 (Pass 1 ↔ Pass 2 contamination check) delivered a new `trading_wiki/extractors/pass2/ablation.py` harness; verdict: keep the priming sentence in both v1 prompts, Pass 1 routing labels are sound. Item 2 (TE silent-price-rescaling audit + prompt v2) delivered `trading_wiki/extractors/pass2/price_audit.py` and `prompts/pass2_trade_example_v2.md`; audit surfaced silent-rescaling anomalies across a subset of TE rows; prompt v2 locked at `pass2-trade-example-v2` after ablation validation. Both Pass 2 extractors now accept optional `prompt_path` / `prompt_version` / `persist` kwargs (defaults preserve locked v1 behavior). Corrective slice re-extracted all 11 baseline TE chunks under v2 ($0.17 LLM cost); v1 rows preserved alongside v2; v2 audit clean except for 2 high findings on chunk 12 — both confirmed false positives caused by Whisper dropping decimal points in spoken stock prices (carried to v0.4 backlog: tighten audit to context-check for spoken-decimal artifacts). Review-UI dev tool shipped 2026-04-26 as a separate slice. Tier 1 corpus = 12 videos / 95 chunks / 12 v1 trade_examples + 9 v2 trade_examples / 112 concepts at locked `pass1-v1`, `pass2-trade-example-v1`+`v2`, `pass2-concept-v1`. Pass 2 idempotency fix shipped 2026-04-29 via `pass2_runs` table. Review UI: local Streamlit page at `trading_wiki/review/app.py`. 286 unit tests + 3 integration tests.*
...
**Status:** 🟠 In progress (Phase 2A v0.1 + v0.2 + v0.2.1 + v0.2.2 SHIPPED 2026-04-25 / 2026-04-26 / 2026-04-28 / 2026-04-29; v0.3 backlog items 1 and 2 of 6 — Pass 1 ↔ Pass 2 contamination check + TE silent-price-rescaling audit & prompt v2 — both SHIPPED 2026-05-10; v0.3 corrective slice — re-extracted all 11 baseline TE chunks under prompt v2 — SHIPPED 2026-05-11; corpus = 12 videos / 95 chunks / 12 v1 + 9 v2 trade_examples / 112 concepts; v0.3 remainder = Strategy/Setup/Rule/MarketCondition + Concept dedup + 4 remaining prompt-iteration backlog items; v0.4 backlog now includes the Whisper-decimal-drop audit false-positive class)
...
### Phase 2
- **Phase 2C v2 Chart extraction (video scene-change frames) MVP — SHIPPED 2026-05-11.** Migration `0013-charts.sql` adds the `charts` table (dedup keyed by `image_hash` so re-runs are idempotent). New module `trading_wiki/pass2c/chart_extractor.py` uses ffmpeg's `scene` filter to detect scene-change timestamps, extracts each as a JPEG, calls Sonnet 4.6 with image input + tool_use to classify chart vs non-chart and extract ticker / timeframe / date_range / indicators / annotations / pattern_description, persists to `charts` with the source frame stored at `storage/charts/<hash>.jpg` (gitignored). Frames are linked to their containing Pass-1 chunk via timestamp overlap (`source_chunk_id`). Locked at `pass2c-chart-v1`. CLI: `python -m trading_wiki.pass2c.chart_extractor --content-id N --max-frames K --threshold T`. **Smoke runs:** content_id=7 @ threshold=0.4 produced 1 scene frame, 0 charts ($0.009); content_id=1 @ threshold=0.25 / max-frames=8 produced 4 scene frames, 0 charts ($0.020). Pipeline is end-to-end functional but **scene-threshold tuning carried forward** — the 5 sampled frames came back as non-chart, suggesting the scene filter is biased toward shot transitions (talking-head ↔ chart cuts) rather than chart-mid-explanation. A fixed-interval sampler (e.g., every 30s) plus dedup against scene-change frames is one likely v0.4 follow-up; another is lowering threshold further. The infrastructure (ffmpeg + vision API + persistence + chunk linking) is solid; charts-found-per-video on this corpus needs the tuning slice. **Phase 2C v1** (PDF/EPUB/Discord image sources) shares the same `charts` table + vision classifier; it ships when image-source content is ingested into the v1 corpus.
- **Phase 2A Pass 4 Relationship Building MVP — SHIPPED 2026-05-11.** Closes the 4-pass extraction pipeline. New migration `0011-entity-relationships.sql` adds `entity_relationships` (polymorphic triples; SQLite doesn't express polymorphic FKs so application code validates refs against the chunk's entity manifest) + `pass4_runs` idempotency table. Eight allowed predicates: `uses` / `prerequisite_for` / `variant_of` / `contradicts` / `supports` / `depends_on` / `illustrates` / `applies_in`. New prompt `prompts/pass4_relationship.md` locked at `pass4-v1`; rules constrain the LLM to triples whose subject/object IDs both come from the provided per-chunk entity manifest and whose support traces back to the chunk text. New module `trading_wiki/extractors/pass4/relationship_builder.py` — per-chunk extractor (skips chunks with <2 entities), content-level driver, CLI. Sonnet 4.6 model. **Live run across all 12 content_ids:** 53 multi-entity chunks → **280 relationships persisted, 0 invalid refs** (~$0.91 Sonnet cost). Predicate distribution: `supports` 119 / `uses` 72 / `prerequisite_for` 46 / `variant_of` 14 / `contradicts` 12 / `depends_on` 11 / `illustrates` 4 / `applies_in` 2. Type-pair distribution: concept→concept 113, rule→rule 76, strategy→rule 26, rule→setup 25, strategy→setup 16, etc. — meaningful graph structure that maps directly onto the Phase 3 wiki layer's expected backlinks. MVP works on candidate-level Pass 2 entities; the relationship rows reference candidate IDs and will need re-keying to canonical IDs once Pass 3 is generalized for non-Concept entity types. 293 unit + 3 integration tests; full regression clean.
- **Phase 2A Pass 2 Rule + MarketCondition extractors — SHIPPED 2026-05-11.** Completes the Pass 2 entity set (all 6 types from PROJECT_PLAN.md schema now operational). Migration 0010 adds `rules` + `market_conditions` tables and widens `pass2_runs.extractor` CHECK to include both. **Rule** extractor routes on `strategy` AND `psychology` Pass 1 chunks (`"rule": {"strategy","psychology"}`); each Rule has a name, statement, optional rationale, and a `category` enum (`trading` / `risk` / `mindset` / `process`). Locked at `pass2-rule-v1`. **MarketCondition** extractor routes on `market_commentary` chunks; each MarketCondition has a label, description, optional time_window, and list of affected_instruments. Locked at `pass2-market-condition-v1`. New prompts `prompts/pass2_rule.md` + `prompts/pass2_market_condition.md`. New modules `trading_wiki/extractors/pass2/rule.py` + `market_condition.py`. Dispatcher gains 2 more independent if blocks. **Live run across all 12 content_ids:** 100 Rules + 1 MarketCondition (the corpus has only 2 market_commentary chunks total — one yielded a regime tag, one did not — appropriate yield). Total Pass 2 cost across this slice: ~$0.41 (Sonnet 4.6). **Final Pass 2 entity counts (Tier 1 corpus, 12 videos / 95 chunks):** 12 v1 TE + 9 v2 TE / 112 Concepts / 9 Strategy candidates / 19 Setup candidates / 100 Rules / 1 MarketCondition. Cross-chunk merging for Strategy / Setup / Rule (and TE) is deferred to a follow-up slice that generalizes the Pass 3 Concept resolver. The `test_skips_unrouted_labels` regression now uses only `noise` since every other Pass 1 label routes somewhere.
- **Phase 2A Pass 2 Setup extractor — SHIPPED 2026-05-11.** Parallel to Strategy, routes on the same `strategy`-labeled Pass 1 chunks (PASS2_LABEL_ROUTES gains `"setup": {"strategy"}`). Dispatcher refactored from chained elif → independent if blocks so a single chunk now invokes both Strategy and Setup extractors; `chunks_routed` counts unique chunks, not extractor invocations. Migration 0009 adds the `setups` table + widens `pass2_runs.extractor` CHECK to include 'setup'. New prompt `prompts/pass2_setup.md` locked at `pass2-setup-v1`; rules clearly distinguish Setup (preconditions + trigger, no exits) from Strategy (full playbook). New module `trading_wiki/extractors/pass2/setup.py` mirrors `strategy.py`. **Live run on all 8 `strategy`-chunk content_ids:** 21 chunks → **19 Setup candidates** persisted (~$0.25 Sonnet cost). Extracted Setups include the v1 source content's recurring patterns (Range Breakout with Options Flow Confirmation, Pivot Confirmation Next-Candle Entry, Top/Bottom Candle Confirmation, etc.); several near-duplicates that will collapse under a future Pass 3 Setup resolver. 142 pass2/db tests pass.
- **Phase 2A Pass 2 Strategy extractor — SHIPPED 2026-05-11.** New per-chunk extractor `trading_wiki/extractors/pass2/strategy.py` follows the v0.2 TradeExample / Concept pattern: structured-output via Anthropic tool_use, idempotency via `pass2_runs`, prompt locked at `pass2-strategy-v1`. New migration `0007-strategies.sql` (table) + `0008-pass2-runs-widen-extractor.sql` (widens the `pass2_runs.extractor` CHECK to allow `'strategy'`). New prompt `prompts/pass2_strategy.md` defines a Strategy as a *named, codeable playbook* (thesis, entry rules, exit rules, indicators, timeframe, instruments, codeability_score 1-5) and explicitly bounds the entity vs. TradeExample (a specific past trade) and Concept (pure definition). The dispatcher gets a third route: `strategy` Pass 1 label → Strategy extractor. **Live run on all 8 content_ids with `strategy` chunks:** 21 chunks routed → **9 Strategy candidates** persisted (~$0.15 Sonnet cost). About half of routed chunks correctly produced 0 entities (the chunk mentioned a setup or playbook in passing but didn't lay out a complete trade rule set). Extracted names cluster around the v1 source content's recurring playbooks; codeability scores fall in 2-3 (mostly judgment with rule-able skeletons), which is realistic for discretionary trading content. Each Strategy row here is a *candidate* drawn from one chunk — cross-chunk merging into canonical Strategy entities (multiple `strategy`-labeled chunks describing the same playbook) is a follow-up slice that generalizes the Pass 3 Concept resolver pattern. Setup, Rule, MarketCondition extractors deferred to subsequent slices (each needs its own routing decision + prompt + table). Tests still 293 unit + 3 integration (Strategy extractor mirrors the TradeExample / Concept patterns already covered by dispatcher + extractor unit tests; the live run is the integration evidence).
- **Phase 2A Pass 3 Concept resolution MVP — SHIPPED 2026-05-11.** Built the entity-resolution machinery as a Concept-only slice (TradeExample / Strategy / Setup / Rule defer to later slices). Stack: OpenAI `text-embedding-3-small` (1536 dim) for embeddings, `sqlite-vec` vec0 virtual table for KNN similarity search, Anthropic Opus 4.7 for pairwise verification (tool_use-enforced schema returning `same`/`different`/`unclear` + reason). New migration `0006-concept-resolutions.sql` adds a regular SQL table `concept_resolutions` that records canonical_concept_id per resolved concept; the vec0 virtual table is created lazily at runtime (SQL migrations can't load the sqlite-vec extension). New module `trading_wiki/extractors/pass3/concept_resolver.py` with `embed_concepts`, `find_candidate_pairs`, `verify_pair_with_llm`, `resolve_concepts` (drives the pipeline + union-find merging so transitively-similar clusters collapse to one canonical_id = min(group)), plus a CLI with `--dry-run`, `--threshold`, `--prompt-version`, `--db-path`. Both API clients injected for testability; 6 unit tests mock OpenAI + Anthropic and verify the full pipeline end-to-end. **Production run at threshold 0.80, top_k 20:** 112 concepts → 4670 input tokens for embeddings (~$0.001) → 36 candidate pairs → 36 Opus verifications ($0.34) → **29 verified `same`, 7 verified `different`**. Eight non-trivial canonical groups emerged, including the 4 PS60-principle duplicates surfaced by the v0.3 #4/#5 investigation (concepts 17, 40, 71, 99) correctly collapsing under canonical=17, and an 8-member "supply / supply zone / supply zones" cluster collapsing under canonical=5. (One member of that cluster — id=110 "supply" with a distinct definition about upside targets — was correctly held out by Opus, validating that the design distinguishes term-collision from concept-duplication.) Tests grew by 7 (1 migration + 6 resolver). 293 unit + 3 integration. **v0.3 #6 acceptance test passed:** Pass 3 found and merged the duplicate Concept rows the investigation predicted.
- **Phase 2A v0.3 backlog item 6 of 6 = Concept synonym dedup — SUBSUMED INTO PASS 3, 2026-05-11.** A standalone Concept dedup slice would require either embedding-based similarity search (needs `sqlite-vec` vec0 + an embedding-model choice) or LLM-driven pairwise verification of all concept pairs — both are pieces of Pass 3 entity resolution. The 4 PS60-principle duplicates surfaced by v0.3 #4/#5 investigation (concepts id=17, 40, 71, 99 — all variants of "stocks always move from supply to supply and demand to demand") and the 2 "supply" duplicates (id=95 and id=110) are the natural acceptance test for Pass 3. Building one-off Concept dedup infrastructure now and discarding it when Pass 3 lands would be wasted work. **Action:** v0.3 #6 closes; Pass 3 picks up the 4+2 known duplicates as its first acceptance test.
- **Phase 2A v0.3 backlog items 4 + 5 of 6 = concept-vs-metaphor + term-naming bias — INVESTIGATED, BUNDLED, NO PROMPT CHANGE, SHIPPED 2026-05-11.** Items 4 and 5 turned out to be the same failure mode under two names: the Concept extractor preserves the speaker's verbose / metaphorical / principle-stated-as-full-sentence phrasing as the `term` field rather than condensing to a canonical short form. Survey of the 112 v1 Concept rows: 7 (6%) had `term` values with ≥5 words. Of those 7: (a) **4 were the same PS60 principle stated 4 different ways** across 4 chunks — "stocks trade from supply to supply", "stocks trade from supply to supply and demand to demand", "theory that stocks go from supply to supply and demand to demand", "stocks always move from supply to supply and demand to demand" (this is a **synonym-dedup problem**, not solvable by a prompt rule alone — covered by v0.3 #6); (b) 2 were metaphor-as-term (the speaker's colorful framing preserved as the term name); (c) 1 was a verbose descriptive-phrase for a concept that has a canonical short form ("short-term expiration deep out of the money calls" → "deep OTM short-dated calls"). Adding a precautionary prompt rule like "prefer shortest canonical term, condense speaker metaphor / verbose phrasing into the definition field" is **risky on a corpus this small** — it would re-extract every Concept row with a different LLM judgment about canonicalization, and per memory's `small_n_prompt_iteration` rule, prompt rewrites on tiny corpora without observed correctness failures are bad ROI. **Carried to v0.4 backlog:** (a) Concept prompt v2 with explicit term-naming + metaphor-canonicalization rule, validated by ablation against same n=10 stratified sample; (b) optional re-extract under v2 (like the v0.3 corrective slice for TE) if ablation passes. The signal is real but small (~6%) and intertwined with the dedup work in v0.3 #6 below. Zero code change, zero LLM cost. Tests still 286 unit + 3 integration.
- **Phase 2A v0.3 backlog item 3 of 6 = trade-attribution semantics — INVESTIGATED, NO PROMPT CHANGE, SHIPPED 2026-05-11.** The v0.3 #2 spec flagged "wrong-trade attribution" (extracted price belongs to a different trade in the same chunk) as needing LLM-assisted verification in a separate slice. Investigation on the current v1 corpus: grep for multi-trade language ("second trade", "two trades", "first trade", "next trade", "another trade", "two different") in `example`-labeled chunks surfaced only chunk 13 across all 11 baseline TE chunks. Chunk 13 contains 2 CRWV trades that were correctly extracted by v2 — 2 distinct TE rows with chunk-faithful descriptions and (correctly) NULL price fields since the speaker didn't state exact prices. No attribution mixup found. Per memory's `small_n_prompt_iteration` rule, no v3 prompt rewrite on a tiny-n no-failure signal. **Action carried to v0.4 backlog:** build an LLM-assisted attribution audit harness (similar shape to `price_audit.py` but Sonnet-driven: ask the LLM to verify each TE row's fields all describe the same trade as the row's ticker/direction) and re-evaluate once the corpus contains more multi-trade content. The current v2 prompt's existing rules — explicit "I had two trades" example, "two trades on the same ticker the same day → keep separate" — are adequate against the observed corpus. Zero code change, zero LLM cost. Tests still 286 unit + 3 integration.
- **Phase 2A v0.3 corrective slice — SHIPPED 2026-05-11.** Re-extracted all 11 baseline TradeExample chunks (12 v1 rows live on 11 distinct source_chunk_ids — one chunk had 2 TE rows) under the locked `pass2-trade-example-v2` prompt. v1 rows preserved alongside v2 rows ("latest version wins" downstream semantics — no schema change; existing `trade_examples.prompt_version` per row + `pass2_runs (chunk, extractor, prompt_version)` UNIQUE already supports multi-version corpora). Total cost: $0.17 at Sonnet 4.6. Result: 12 v1 rows unchanged; 9 v2 rows persisted (3 chunks correctly dropped to 0 entities — 24 misattributed options-strike-as-target, 74 fabricated price refused, 92 a new borderline drop v2 declined that v1 had extracted); 11 v2 `pass2_runs` rows. New one-off driver `trading_wiki/extractors/pass2/corrective_reextract.py` discovers baseline chunks, drives per-chunk re-extraction via the existing `extract_trade_examples_for_chunk` function with the v2 prompt path/version, aggregates a `RunResult` summary (per-chunk v1/v2 counts + v2 entity price snapshots + cost), writes `summary.md` + `summary.json` under gitignored `data/corrective/<run_id>/`. `price_audit.py` generalized with a `prompt_version` parameter + `--prompt-version` CLI flag (default = locked v1 so existing invocations unchanged); findings.json/.md now carry the audited prompt_version so v1 vs v2 artifacts disambiguate. Acceptance: **v2 audit found 2 high-severity on chunk 12 (both fields on a single TE row); both confirmed false positives** — the audit's ×100 pattern flagged `12.5` and `14.45` because the Whisper transcript captured "twelve fifty" and "fourteen forty-five" as `1250` and `1445` (decimal dropped in spoken stock prices). Context in the same chunk text confirms the speaker means dollars in the $12–14 range ("the stock went from 1250 to all the way up to almost 14"). v1 and v2 both correctly interpret these as $12.50 / $14.45; v2 is not a regression. **Carried to v0.4 backlog:** tighten the audit to recognize Whisper-decimal-drop false positives via local-context heuristics (look for "almost N" / "up to N" / "from N to" near the spoken-decimal candidate). Tests grew by 7 (1 v1-filter, 1 CLI v2-filter, 1 artifact-writer prompt_version, 1 discover, 1 run aggregation, 1 artifact writer, 2 CLI subprocess). 286 unit + 3 integration; ruff + ruff-format + mypy + gitleaks all clean.
- **Phase 2A v0.3 backlog item 2 of 6 = TE silent-price-rescaling audit + prompt v2 — SHIPPED 2026-05-10.** New `trading_wiki/extractors/pass2/price_audit.py` is a read-only audit that walks each locked-v1 TradeExample row's price fields and scans the source chunk text for literal + ×10/÷10/×100/÷100 variants of each value. Digit-boundary regex rejects matches inside larger numbers (e.g., a 3-digit integer inside a 4-digit one) and rejects integer matches followed by `.<digit>` so a 3-digit integer doesn't spuriously match a longer decimal price; `$`-stripping + thousands-comma normalization handles unit decoration. Severity: literal-present → info; literal-absent + any rescaled-present → high; nothing-present → medium. Findings written to gitignored `data/audits/<run_id>/{findings.json,findings.md}`. Audit run on the 12-row v1 baseline surfaced 3 high-severity silent-rescaling anomalies (two ×100, one ÷100) across 2 distinct TE rows, and 2 medium-severity rows where the extracted price is absent from the source chunk text entirely (paraphrase or fabrication). Prompt revision (`prompts/pass2_trade_example_v2.md`) adds one explicit "Preserve numeric scale" bullet under Edge cases (priming sentence — proven load-bearing by v0.3 #1 — untouched). Ablation harness (`ablation.py`) generalized with `--arms te,concept,routing` filter and per-arm `--{te,concept}-test-prompt-{path,version}` overrides defaulting to the existing blind paths so the contamination use case is unchanged; `AblationConfig` and the `config.json` artifact now record which paths were actually used so artifacts disambiguate blind runs from prompt-revision validation runs. v2 validated by re-running the harness against the same n=10 stratified sample / seed=42 as v0.3 #1, TE arm only, $0.11 cost. **Verdict: pass — v2 locked at `pass2-trade-example-v2`.** Inside the sampled subset: identical=5 (all baseline=0/v2=0); 4 chunks with field-level changes that all represent v2 catching v1 mistakes (one ÷100 rescaling corrected to its literal value; one chunk where v2 captured a target_price the speaker explicitly stated that v1 missed; one chunk where v2 dropped a misattributed options-strike-as-target; one chunk where v2 dropped a year v1 had invented for a no-year date); 1 count_changed where v2 correctly declined to extract a fabricated price (a medium-severity audit row that fell in the sample). The remaining baseline anomalies outside the v2 sample (1 row with both fields wrong, 1 medium row with a no-text price) are **carried to a follow-up corrective slice** (re-extract or hand-fix the affected rows; design and scope TBD in that slice). Tests: 279 unit + 3 integration.
- **Phase 2A v0.3 backlog item 1 of 6 = Pass 1 ↔ Pass 2 contamination check — SHIPPED 2026-05-10.** New ablation harness at `trading_wiki/extractors/pass2/ablation.py` — pulls a stratified sample (n=10 per arm: example chunks for TE priming, concept/qa chunks for Concept priming, non-Pass2-routed labels for routing audit), runs both extractors with "blind" prompt variants (priming sentence stripped) at locked-but-distinct `pass2-*-blind-v1` versions, diffs against baseline DB rows, and writes 4 markdown/JSON artifacts under gitignored `data/ablation/<run_id>/`. Both `extract_trade_examples_for_chunk` and `extract_concepts_for_chunk` were parameterized with optional `prompt_path` / `prompt_version` / `persist` kwargs (defaults preserve locked v1 behavior; `persist=False` skips both the idempotency check and DB writes). Run at n=10/10/10, seed=42 cost $0.34. **Verdict:** **keep the priming sentence in both v1 prompts.** Concept extractor changed on 10/10 chunks under blind prompt — 5 with different concept counts (added/removed terms, not just rephrased), 5 with definition + related_terms drift; TE was more robust at 5/10 identical but had field-level drift on 4 chunks (paraphrasing of `outcome_text` / `lessons`, two `target_price` losses, one `entry_price` scale anomaly 295.0 → 2.95 in blind output) plus 1 count_changed (blind dropped one trade). **Pass 1 routing labels are sound at n=10** — the single routing-arm "miss" was blind-prompt over-extraction on a strategy-talk chunk where the speaker described a setup but explicitly said no trade was taken. Routing-by-label is safe; no Pass 1 prompt change needed. **Two follow-ups carried to v0.4 backlog:** (a) TE entry-price scale spot-check across the v1 corpus; (b) optional Concept-prompt revision to test whether a more explicit definition-vs-mention rule reduces extraction volatility. Mid-slice fix (commit `94b93e6`): the diff function inflates rendered `changed_fields` with DB-internal noise unless the CLI strips them first. Tests now 237 unit + 3 integration.
