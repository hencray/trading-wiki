...
**Pass 2 Strategy extractor shipped 2026-05-11.** New per-chunk extractor `trading_wiki/extractors/pass2/strategy.py` follows the v0.2 TE/Concept pattern. Migrations 0007 (strategies table) + 0008 (pass2_runs CHECK widening). Locked at `pass2-strategy-v1`. Live run: 21 `strategy`-labeled chunks routed → 9 Strategy candidates persisted (~$0.15). Cross-chunk merge into canonical Strategy entities deferred to a follow-up slice that generalizes the Pass 3 Concept resolver. 293 unit + 3 integration tests.
