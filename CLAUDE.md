...
## Current State: Phase 2A v0.3 closed + Pass 3 Concept resolution MVP SHIPPED; Strategy/Setup/Rule + Pass 4 + 2B + 2C remain

**Pass 3 MVP shipped 2026-05-11.** New stack: OpenAI text-embedding-3-small (1536 dim) → sqlite-vec vec0 KNN → Opus 4.7 pairwise verification. New migration `0006-concept-resolutions.sql` + module `trading_wiki/extractors/pass3/concept_resolver.py`. Production run on 112 concepts at threshold 0.80 produced 36 candidate pairs → 29 `same` + 7 `different` ($0.34 Opus cost). Eight canonical groups including all 4 PS60-principle duplicates merged correctly under canonical=17. 293 unit tests + 3 integration.
