...
## Current State: Phase 2A v0.2.2 shipped; Phase 2A v0.3 next

As of 2026-04-29, Phase 1 (ingestion) and Phase 2A v0.1 + v0.2 + v0.2.1 + v0.2.2 are done and committed. Corpus: 12 videos / 95 chunks / 12 trade_examples / 112 concepts at locked prompt versions `pass1-v1`, `pass2-trade-example-v1`, `pass2-concept-v1`. v0.2.1 (2026-04-28) ingested 10 more Tier 1 videos and shipped a fix for Pass 1's coverage-retry path. v0.2.2 (2026-04-29) shipped a Pass 2 idempotency fix via a new `pass2_runs` table — both extractors previously gated idempotency on entity-row presence, which couldn't distinguish "ran with 0 entities" from "never ran"; the new table records every run with its entity count, so empty-result calls are now properly idempotent. Repo: **`hencray/trading-wiki`** on GitHub (public, MIT). 210 unit tests + 2 integration tests.

**Next slice — Phase 2A v0.3:** Strategy/Setup/Rule/MarketCondition entity extraction (cross-chunk, needs Pass 3 entity-resolution scaffolding) + Concept dedup + the v0.2 prompt-iteration backlog (see `PROJECT_PLAN.md` Phase 2 decisions: silent price rescaling, trade-attribution semantics, concept-vs-metaphor classification, term-naming bias, synonym dedup, Pass 1 ↔ Pass 2 contamination check). The prompt-iteration backlog gate (n=2 was "too small") is now unblocked at n=12 videos.
