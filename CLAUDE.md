...
**Pass 3 Concept resolution MVP shipped 2026-05-11.** OpenAI text-embedding-3-small (1536 dim) → sqlite-vec vec0 KNN → Opus 4.7 pairwise verification. Migration `0006-concept-resolutions.sql` + module `trading_wiki/extractors/pass3/concept_resolver.py`. Production run: 112 concepts → 36 candidate pairs at cosine ≥0.80 → 29 `same` / 7 `different` ($0.34 Opus cost). 8 canonical groups; all 4 PS60-principle duplicates merged correctly. Generalization to Strategy/Setup/Rule/TE deferred.

293 unit + 3 integration tests.
