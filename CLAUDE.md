...
## Current State: Phase 2A v0.3 backlog items 1 and 2 of 6 shipped; remaining v0.3 work next

As of 2026-05-10, Phase 1 (ingestion) and Phase 2A v0.1 + v0.2 + v0.2.1 + v0.2.2 are done and committed. Corpus: 12 videos / 95 chunks / 12 trade_examples / 112 concepts at locked prompt versions `pass1-v1`, `pass2-trade-example-v1`, `pass2-concept-v1`. v0.2.1 (2026-04-28) ingested 10 more Tier 1 videos and shipped a fix for Pass 1's coverage-retry path. v0.2.2 (2026-04-29) shipped a Pass 2 idempotency fix via a new `pass2_runs` table. **v0.3 backlog items 1 and 2 of 6 BOTH SHIPPED 2026-05-10.** Item 1 — Pass 1 ↔ Pass 2 contamination check via a new ablation harness at `trading_wiki/extractors/pass2/ablation.py`. Verdict at n=10/10/10: **keep the priming sentence in both v1 prompts** (Concept extractor changed on 10/10 chunks under blind prompt; TE was more robust at 5/10 identical but had field-level drift on a few); **Pass 1 routing labels are sound** (the single routing-arm "miss" was blind-prompt over-extraction, not a Pass 1 mislabel). Both Pass 2 extractors now accept optional `prompt_path` / `prompt_version` / `persist` kwargs (defaults preserve locked v1 behavior). Item 2 — TE silent-price-rescaling audit + prompt v2. New `trading_wiki/extractors/pass2/price_audit.py` read-only audit flagged 3 high-severity silent rescalings (two ×100, one ÷100) in 2 of the 12 baseline TE rows, plus 2 medium-severity rows where the extracted price isn't in the source chunk at all. New `prompts/pass2_trade_example_v2.md` adds one "Preserve numeric scale" rule (locked at `pass2-trade-example-v2`); ablation harness generalized with `--arms` filter + per-arm test-prompt overrides. v2 ablation against same n=10 / seed=42 sample as v0.3 #1 passed: chunk 26's `295.0 → 2.95` anomaly corrected; chunk 74's fabricated price refused (correct behavior); 3 other improvements (target capture, options-strike rejection, no-year date NULLing) on chunks v1 had gotten wrong in subtler ways. **Follow-up corrective slice** carries the 5 baseline TE rows the audit surfaced (chunk 12 outside the v2 ablation sample, plus the 2 medium rows). Repo: **`hencray/trading-wiki`** on GitHub (public, MIT). 279 unit tests + 3 integration tests.

**Next slice — Phase 2A v0.3 remainder:** Strategy/Setup/Rule/MarketCondition entity extraction (cross-chunk, needs Pass 3 entity-resolution scaffolding) + Concept dedup + the remaining 5 prompt-iteration backlog items (silent price rescaling, trade-attribution semantics, concept-vs-metaphor classification, term-naming bias, synonym dedup). The contamination-check slice carried a follow-up (a) — TE entry-price scale spot-check across the v1 corpus — which v0.3 #2 just landed (3 high + 2 medium anomalies surfaced, v2 locks the rule for future-proofing, the 5 affected rows go to a corrective follow-up slice). Carried forward: (b) optional Concept-prompt revision in v0.4 to test whether a more explicit definition-vs-mention rule reduces the high extraction volatility observed under the blind variant; and the new corrective slice for the 5 baseline TE rows.
