...
- **Preserve numeric scale.** Record each price field exactly as the speaker
  stated it. If the speaker says "two ninety-five," write `295.0`. If they
  say "two ninety-five dollars," write `295.0` and capture the unit phrasing
  in `entry_description` / `exit_description`. If the speaker corrects
  themselves mid-sentence ("295 — I mean 2.95"), use the final stated value.
  If the scale is genuinely ambiguous ("in the high two hundreds" or no
  clear unit), set the price field to NULL and put the speaker's exact
  words in the description field. **Never apply a unit conversion or
  rescale the value yourself.**
